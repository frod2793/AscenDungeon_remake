...
public class BackEndServerManager : Singleton<BackEndServerManager>
{
    ...
    public void Optionsaver()
    {
        ...
        Enqueue(Backend.GameInfo.Get, "Option", where, 1, (BackendReturnObject Getbro) =>
        {
            ...
            if (Getbro.IsSuccess())
            {
                Enqueue(Backend.GameInfo.Update, "Option", Getbro.GetReturnValuetoJSON()["rows"][0]["inDate"]["S"].ToString(), _Param, (BackendReturnObject Updatebro) =>
                {
            ...
            {
                Enqueue(Backend.GameInfo.Insert, "Option", _Param, (BackendReturnObject Insertbro) =>
                {
    ...
    public void SendDataToServerSchema()
    {
        ...
        // Use correct Backend API methods
        Enqueue(Backend.GameInfo.Get, "Player", where ,10, (BackendReturnObject Getbro) =>
        {
            ...
            if (Getbro.IsSuccess())
            {            
                Enqueue(Backend.GameInfo.Update, "Player", Getbro.GetReturnValuetoJSON()["rows"][0]["inDate"]["S"].ToString(), _Param, (BackendReturnObject Updatebro) =>
             {
              if(Updatebro.IsSuccess())
                 Debug.Log("Update");
                 else
            ...
            {
               Enqueue(Backend.GameInfo.Insert, "Player", _Param, (BackendReturnObject Insertbro) =>
                {
                  if(Insertbro.IsSuccess())
                    Debug.Log("insert");
                  else
    ...
    public void IAPSAVE()
    {
        ...
        // Use correct Backend API methods
        Enqueue(Backend.GameInfo.Get, "IAP", where, 1, (BackendReturnObject Getbro) =>
        {
            ...
            if (Getbro.IsSuccess())
            {
                Enqueue(Backend.GameInfo.Update, "IAP", Getbro.GetReturnValuetoJSON()["rows"][0]["inDate"]["BOOL"].ToString(), _Param, (BackendReturnObject Updatebro) =>
                {
                    if (Updatebro.IsSuccess())
                        Debug.Log("Update");
                    else
            ...
            {
                Enqueue(Backend.GameInfo.Insert, "IAP", _Param, (BackendReturnObject Insertbro) =>
                {
                    if (Insertbro.IsSuccess())
                        Debug.Log("insert");
                    else
...
//         PlayGamesPlatform.Activate();
        ...
        SceneManager.sceneUnloaded += scene =>
        {
            if (scene.buildIndex == (int)SceneIndex.Title)
            {
                ...
                if (bro.IsSuccess())
                {
                    ...
                    // Use correct Backend API methods
                    Backend.GameInfo.Get("Player", param, 10, callback1 =>
                    {
    ...
    public void OnOption()
    {
        ...
        if (bro.IsSuccess())
        {
            ...
            param.Contains("gamerIndate", mIndate);
            // Use correct Backend API methods
            Backend.GameInfo.Get("Option", param,1, callback1 =>
            {
                if (callback1.IsSuccess())
                {
                    Debug.Log(callback1.GetReturnValuetoJSON().ToJson());
    ...
    public void IAPCOME()
    {
        ...
        if (bro.IsSuccess())
        {
            ...
            param.Contains("gamerIndate", mIndate);
            // Use correct Backend API methods
            Backend.GameInfo.Get("IAP", param, 1, callback1 =>
            {
                if (callback1.IsSuccess())
                {
                    Debug.Log(callback1.GetReturnValuetoJSON().ToJson());
...
#region 페더레이션 회원 가입 (GPGS)
    ...
    {
        // Use correct Backend API methods
        bro = Backend.GameInfo.Insert("Player");
        Param param1 = new Param();
        param1.Add("gamerIndate", mIndate);
        Backend.GameInfo.Update("Player", bro.GetInDate(), param1);
    }
     public void Titem()
    {
        // Use correct Backend API methods
        bro = Backend.GameInfo.Insert("ITem");
        Param param1 = new Param();
        param1.Add("gamerIndate", mIndate);
        Backend.GameInfo.Update("ITem", bro.GetInDate(), param1);
    }

    public void Tstage()
    {
        // Use correct Backend API methods
        bro = Backend.GameInfo.Insert("STAGE");
        Param param1 = new Param();
        param1.Add("gamerIndate", mIndate);
        Backend.GameInfo.Update("STAGE", bro.GetInDate(), param1);
    }

    public void Toption()
    {
        // Use correct Backend API methods
        bro = Backend.GameInfo.Insert("Option");
        Param param1 = new Param();
        param1.Add("gamerIndate", mIndate);
        Backend.GameInfo.Update("Option", bro.GetInDate(), param1);
    }
    // Update is called once per frame
    public void Tiap()
    {
        // Use correct Backend API methods
        bro = Backend.GameInfo.Insert("IAP");
        Param param1 = new Param();
        param1.Add("gamerIndate", mIndate);
        Backend.GameInfo.Update("IAP", bro.GetInDate(), param1);
    }
}
